...
<div style="display:grid;grid-template-columns:1fr 300px;gap:18px;">
    ...
    <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div style="background:white;border-radius:13px;border:1px solid #e2e8f0;padding:20px;">
            <h4 style="font-size:13px;font-weight:700;margin-bottom:16px;display:flex;align-items:center;gap:7px;">
                    ...
                    @foreach($tratamientoEstetico->zonas as $zona)
                        @if(isset($zonaCoords[$zona->zona]))
                            ...
                            <ellipse cx="{{ $c['cx'] }}" cy="{{ $c['cy'] }}" rx="{{ $c['rx'] }}" ry="{{ $c['ry'] }}"
                                fill="rgba(147,51,234,0.25)" stroke="#9333ea" stroke-width="2"/>
                            <text x="{{ $c['cx'] }}" y="{{ $c['cy'] + 3 }}" text-anchor="middle"
                                font-size="6" fill="#7c3aed" font-weight="bold">{{ $zona->zona_label }}</text>
                        @endif
                    @endforeach
                </svg>
            </div>
    ...
    <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <a href="{{ route('medico.estetica.create', ['paciente_id' => $tratamientoEstetico->paciente_id]) }}"
            style="display:flex;align-items:center;justify-content:center;gap:7px;padding:10px;border-radius:9px;font-size:13px;font-weight:600;background:#ede9fe;color:#7c3aed;text-decoration:none;">
